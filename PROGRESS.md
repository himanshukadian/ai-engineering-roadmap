...
- **M6 (PENDING)** Deploy: single GitHub release `emotion-v1.0.0` (upload `emotion_baseline.pt` + `emotion_cbam_aspect.pt` ← rename of notebook's `emotion_cbam_ac.pt`), app auto-downloads to `checkpoints/`, live Streamlit URL, README/NOTES, commit (only with approval).
  - **M6 status: DONE** — release `emotion-v1.0.0` created with all 5 assets (3 checkpoints + temps + centroids, verified downloadable); code committed+pushed (`1272823`, then `caa1a03` adds prior-correct toggle); **live app: https://resnetemotiondetection.streamlit.app/** (main file `computer_vision/classification/emotion_detection/app.py`); README "Try it live" mirrored.
  - **Prior-correct toggle (M6 add-on, shipped)**: `core.PRIORS` (class priors implied by capped CE weights; neutral 34.2% / contempt 1.5% → 22.4× rare boost), `predict/analyze(prior=)` = `softmax(logits/T − log ρ)`; app checkbox + caption. Measured on the 5 photos: does NOT flip c/d/man-2 (8–15 logit gap vs ~1.5 correction) — verified honest, no flips.
  - **Loop closed (Sept 2026)**: user reports errors on OTHER classes (subtle disgust/surprise) too → root cause is broader than closed-mouth anger: an **expression-intensity gap** (benchmark datasets over-sample strong mouth-open versions of every emotion; subtle personal-photo variants collapse toward each other on the neutral manifold, same MAR mechanism measured on anger). Only measured on anger (2–3 photos/emotion for a full matrix not yet supplied). Fix direction unchanged and recorded: personal-photo fine-tuning (M8, parked pending decision) + optionally AffectNet-mined closed-mouth anger via MAR filter (dataset search done: HF `chitradrishti/AffectNet` ~8.7GB, AffectNet+ soft-label Difficult subset, ExpW).
